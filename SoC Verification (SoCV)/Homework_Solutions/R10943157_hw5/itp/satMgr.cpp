...
void SATMgr::itpUbmc(const V3NetId& monitor, SatProofRes& pRes) {
   ...
   // Solve whether the Counter Example has found at timeframe 0
   if(satSolver->assump_solve()) {
      pRes.setFired(0);
   ...
   int Lsize = _ntk -> getLatchSize();
   for(int i=0; i < Lsize; i++){
      V3NetId LatchID = _ntk -> getLatch(i);
      satSolver -> addBoundedVerifyData(LatchID, 1);
      Var _var = satSolver -> getVerifyData(LatchID, 1);
      mapVar2Net(_var, LatchID);
   } 
 
   int Nclause = getNumClauses();   
...
// PART III:
   ...
   int K_Curr = 1;
   for( ; K_Curr < pRes.getMaxDepth(); K_Curr++ ){

      S_ID = getNumClauses(); 
      satSolver -> assumeRelease(); 
      satSolver -> assumeProperty(I, false, 0 ); 
      satSolver -> addBoundedVerifyData(monitor, K_Curr); 
      satSolver -> assumeProperty(monitor, false, K_Curr); 
      satSolver -> simplify();

      if(!(satSolver->assump_solve())) 
      {
 
         for(unsigned i=S_ID; i<getNumClauses(); i++)
         {
            markOffsetClause(i);
         }
         V3NetId reachState = I;
    
         V3NetId interpolation = getItp();
         while(true)
         {    
            S_ID = getNumClauses(); 
            satSolver -> addBoundedVerifyData(interpolation, 0);
            ...
            satSolver -> assumeRelease();
            satSolver -> assumeProperty(interpolation, false, 0);
            satSolver -> assumeProperty(monitor, false, K_Curr);
            satSolver -> simplify();
            ...
            if(!(satSolver->assump_solve()))
            {
               ...
               else
               {
                  ...
                  for(int i=S_ID; i<getNumClauses(); i++){
                     markOnsetClause(i);
                  }
                  reachState = newReachState;
               }
            }
            else
            {
               S_ID = getNumClauses();
               satSolver->assertProperty(monitor, true, K_Curr); 
...
V3NetId SATMgr::buildInitState() const 
{
   ...
   I = _ntk->getConst(0);
   I = ~I;
   V3NetId ntkId1,ntkId2;
   uint32_t netSize = _ntk->getNetSize();
   for(unsigned i=0; i< _ntk->getLatchSize(); i++){
      ntkId2 = _ntk -> getLatch(i);
      ntkId1 = I;
      I = _ntk -> createNet();
      createV3AndGate(_ntk, I, ntkId1, ~ntkId2);
   }
