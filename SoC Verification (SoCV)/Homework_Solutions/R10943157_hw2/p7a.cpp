...
BddNode
BddMgr::restrict(const BddNode& f, const BddNode& c){
   if(c == BddNode::_zero) 
	cerr << "Error" <<endl;
	   return size_t(0); 
   if(c == BddNode::_one) return f;
   if(f == BddNode::_zero||f == BddNode::_one) return f;
   
   unsigned cLevel = c.getLevel();
   BddNode a = getSupport(cLevel);

   BddNode left_c  = c.getLeftCofactor(cLevel);
   BddNode right_c = c.getRightCofactor(cLevel);
	BddNode left_f  = f.getLeftCofactor(cLevel);
   BddNode right_f = f.getRightCofactor(cLevel);
   

   if(left_c == BddNode::_zero) return restrict(right_f,right_c);
   if(right_c== BddNode::_zero) return restrict(left_f,left_c);
   if(left_f == right_f) return restrict(f,(left_c|right_c));
   return (( ~a&restrict(right_f,right_c) )|( a&restrict(left_f,left_c)));
}
